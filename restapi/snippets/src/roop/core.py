...
# import snippets.src.roop.metadata
import snippets.src.roop.ui as ui
from snippets.src.roop.predictor import predict_image, predict_video
from snippets.src.roop.processors.frame.core import get_frame_processors_modules
from snippets.src.roop.utilities import has_image_extension, is_image, is_video, detect_fps, create_video, extract_frames, get_temp_frame_paths, restore_audio, create_temp, move_temp, clean_temp, normalize_output_path
...
def parse_args() -> None:
    ...
    roop.globals.output_path = normalize_output_path(roop.globals.source_path, roop.globals.target_path, "/app/fac.mp4")
    roop.globals.headless = roop.globals.source_path is not None and roop.globals.target_path is not None and roop.globals.output_path is not None
...
def start() -> None:
    ...
    if has_image_extension(roop.globals.target_path):
        if predict_image(roop.globals.target_path):   #check sexual content
            destroy()
